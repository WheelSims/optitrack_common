...
user_arm_extended_distance = 0.2752
...
def plot_min_distance_setup(ax):
    ...
    )  # 15 cm au-dessus de l'extension maximale
...
def update_plot(
    ax1, ax2, push_positions, recovery_positions, minimum_recovery_distances
):
    ...
    if len(minimum_recovery_distances) >= 3:
        for i in range(2, len(minimum_recovery_distances), 3):
            ...
            ax2.axhline(
                ...
                linestyle="-",
                linewidth=2,
                label=f"Moyenne en temps réel: {avg_min_distance:.2f}",
            )
            ax2.legend()
